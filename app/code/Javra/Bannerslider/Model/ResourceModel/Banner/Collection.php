...
namespace Javra\Bannerslider\Model\ResourceModel\Banner;
...
class Collection extends AbstractCollection
{
    ...
    {
        $this->_init('Javra\Bannerslider\Model\Banner', 'Javra\Bannerslider\Model\ResourceModel\Banner');
    }
    ...
    public function __construct(
        ...
        \Javra\Bannerslider\Model\Slider $slider,
        $connection = null,
    ...
    public function joinSliderTitle()
    {
        $this->getSelect()->joinLeft(
            ['sliderTable' => $this->getTable('javra_bannerslider_slider')],
    ...
    public function addFieldToFilter($field, $condition = null)
    {
        ...
        if (in_array($field, $attributes) && $storeViewId) {
            if (!in_array($field, $this->_addedTable)) {
                ...
                $this->getSelect()
                    ->joinLeft(array($field => $this->getTable('javra_bannerslider_value')), $sql, array());
    ...
    public function getBannerCollection($sliderId)
    {
        ...
        /** @var \Javra\Bannerslider\Model\ResourceModel\Banner\Collection $bannerCollection */
        $bannerCollection = $this->setStoreViewId($storeViewId)
            ->addFieldToFilter('slider_id', $sliderId)
            ->addFieldToFilter('status', \Javra\Bannerslider\Model\Status::STATUS_ENABLED)
            ->addFieldToFilter('start_time', ['lteq' => $dateTimeNow])
            ->addFieldToFilter('end_time', ['gteq' => $dateTimeNow])
            ->setOrder('order_banner', 'ASC');

        if ($this->_slider->getSortType() == \Javra\Bannerslider\Model\Slider::SORT_TYPE_RANDOM) {
